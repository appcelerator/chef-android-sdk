user 'random' do
  group       'staff'
  manage_home true
  shell       '/bin/bash'
  home        '/Users/random' if platform_family?('mac_os_x')
  # attributes necessary to set password of 'password' on macOS
  iterations  25_000 if platform_family?('mac_os_x')
  salt        '2961f8ef3c544df3b114b7847ed4307382a9a9e6b3c8c252ffa8a932629d4334' if platform_family?('mac_os_x')
  password    '' if platform?('mac_os_x')
end

directory '/Users/random' do
  owner 'random'
  group 'staff'
  only_if { platform_family?('mac_os_x') }
end
