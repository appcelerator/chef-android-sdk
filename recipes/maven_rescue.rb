...
maven_android_sdk_deployer_root = ::File.basename(node['android']['path'])
maven_android_sdk_deployer_home = File.join(maven_android_sdk_deployer_root, node['android']['maven-android-sdk-deployer']['name'])

#
# Install Maven Android SDK Deployer from git repository
#
directory maven_android_sdk_deployer_home do
  user node['android']['owner']
  group node['android']['group']
  mode 00755
  action :create
end
git maven_android_sdk_deployer_home do
  repository node['android']['maven-android-sdk-deployer']['git_repository']
  revision node['android']['maven-android-sdk-deployer']['version']
  checkout_branch "deploy_#{node['android']['maven-android-sdk-deployer']['version']}"
  action :sync
  user node['android']['owner']
  group node['android']['group']
end
...
# it could be nicer/safer to loop over node['android']['components'] and
...
execute 'Execute maven-android-sdk-deployer' do
  command "mvn clean install -Dmaven.repo.local=#{node['android']['maven-local-repository']} --fail-never -B"
  user node['android']['owner']
  group node['android']['group']
  cwd maven_android_sdk_deployer_home

  # FIXME: setting HOME might be required (if $HOME used in node['android']['maven-local-repository'],
